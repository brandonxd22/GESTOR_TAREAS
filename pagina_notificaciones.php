			...
			<div class="container">
				<div class="row">
					<div class="col-md-10 col-md-offset-1 text-center">
						...
						<h1 class="white-text">NOTIFICACIONES</h1>
					</div>
				</div>
			</div>
		</div>	
<?php 
						...
						<td><form  action='pagina_tareas_pendientes.php' method='post' style='float: right'><input type='submit' value='Tareas Completadas' style='width:230px; height:50px;color: white;float: center;font-size:14pt; background-color: grey;border: 3px solid black;font-weight: bold'/></form> </td>
								...
								<br><br><font face='Eras Bold ITC'> <h4 style='text-align: center;font-family:Verdana; font-size: 40px'>Notificaciones</h4></font>
							    ...
							    <font face='Eras Bold ITC'> <h2 style='text-align: center;font-family:Verdana; font-size: 25px;color:red;'>Tienes tareas Pendientes en las proximas fechas:<br></h2></font>";
							    ...
							    while ($row=mysqli_fetch_array($consulta_pendientes_fecha))
							    {
							    	$fecha_tareas=$row['fecha_limite'];
							    	$nombre_tarea=$row['nombre'];
										echo "<font face='Eras Bold ITC'> <h2 style='text-align: center;font-family:Verdana; font-size: 25px;color: blue;'> $nombre_tarea | $fecha_tareas <br></h2></font>";
							    }
							    $consulta_usuarios= mysqli_query($conexion_db,"select * from usuario;");
							    while ($row_usuario=mysqli_fetch_array($consulta_usuarios))
							    {
							    	$id_usuario=$row_usuario['id_usuario'];
							    	$nombre_usuario=$row_usuario['nombre'];
							    
							    $consulta_tarea= mysqli_query($conexion_db,"select tarea.nombre nombre, tarea.fecha_completada from tarea, usuario where usuario.id_usuario= tarea.id_usuario and tarea.estatus = 'COMPLETADA' and tarea.id_usuario=".$id_usuario." and tarea.fecha_completada = '".$fecha_conversion."';");
							    while ($row=mysqli_fetch_array($consulta_tarea))
							    {
							    	$fecha_tareas=$row['fecha_completada'];
							    	$nombre_tarea=$row['nombre'];
							    	
										echo "<font face='Eras Bold ITC'> <h2 style='text-align: center;font-family:Verdana; font-size: 25px;color: green;'>El Usuario ".$nombre_usuario." completó la tarea ".$nombre_tarea." el dia de hoy <br></h2></font>";
