...
<?php 
						...
						<td><form  action='pagina_tareas_pendientes.php' method='post' style='float: right'><input type='submit' value='Tareas Pendientes' style='width:250px; height:50px;color: white;float: center;font-size:14pt; background-color: grey;border: 3px solid black;font-weight: bold'/></form> </td>
						<td><form  action='pagina_tareas_completadas.php' method='post' style='float: right'><input type='submit' value='Tareas Completadas' style='width:230px; height:50px;color: white;float: center;font-size:14pt; background-color: #FF6B4C;border: 3px solid black;font-weight: bold'/></form> </td>
								...
								<br><br><font face='Eras Bold ITC'> <h4 style='text-align: center;font-family:Verdana; font-size: 40px'>Total Tareas Pendientes</h4></font>
							    ...
							    $consulta= mysqli_query($conexion_db,"select id_tarea,nombre, descripcion, estatus, fecha_limite, fecha_creacion,  estatus, observaciones, prioridad from tarea where estatus='En progreso' order by fecha_limite ;");
							    $contador_tareas=0;
